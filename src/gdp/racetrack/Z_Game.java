...
public class Z_Game extends JFrame{
		 ...
		 public void paint(Graphics gi){
				...
				g.setStroke(new BasicStroke(2));			 
			 	...
			 	for (int t=0;t<m.getStartPoints().length;t++){
					kreis(g,m.getStartPoints()[t].getX()*16,m.getStartPoints()[t].getY()*16,Color.YELLOW);
	...
	final RuleSet ST=new RuleSet(new EnvironmentCollisionRule_Bounce(),new PlayerCollision_PhaseThrough(),new TurnRule_J(),new VictoryRule_First());
	P = new HumanPlayer[1];
    ...
    Vec2D vej=new Vec2D(0,0);;
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (ny % 16>8)while (ny % 16!=0){ny++;}
			if (first) {first=false;ox=nx;oy=ny; P[0].chooseStart(startPoints);
			for (int t=0;t<m.getStartPoints().length;t++){
				kreis(g2d,m.getStartPoints()[t].getX()*16,m.getStartPoints()[t].getY()*16,Color.BLACK);}
			} else{
